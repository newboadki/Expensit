...
enum CurrencyCode: String, Hashable {
    case EUR
    case HRK
    case GBP
    case USD
...
typealias CurrencyExchangeMap = [CurrencyCode : [CurrencyCode : Double]]

struct CurrencyExchangeInfo {
    
    let baseCurrencyCode: CurrencyCode
    
    let date: Date
    
    let exchangeRates: CurrencyExchangeMap
}
